...
每轮独立：一次失败会被记录到 `crawl_runs`，并在循环模式下继续等待下一轮；不会让常驻进程退出。成功轮在同一个数据库事务中写入批次状态和 10 条明细，任一写入失败则回滚成功数据并记录失败批次。

## 配置与安全

数据库连接仅从环境变量 `DATABASE_URL` 读取。缺失、无法连接或迁移失败时，程序明确报错，不尝试使用默认数据库或输出凭据。
